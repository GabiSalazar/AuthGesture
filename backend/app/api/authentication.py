"""
API del Authentication System.

Expone endpoints para iniciar verificación/identificación, procesar frames,
consultar estado de sesión, cancelar sesiones y obtener estadísticas/usuarios.
"""
...
class ProcessFrameRequest(BaseModel):
    """Request para procesar un frame (base64) desde frontend"""
...
class AuthenticationStatusResponse(BaseModel):
    """Response del estado de una sesión de autenticación"""
    session_id: str
    mode: str
    user_id: Optional[str]
    status: str
...
async def start_verification(request: VerificationStartRequest):
    """
    Inicia una sesión de verificación biométrica 1:1.

    Args:
        request (VerificationStartRequest): datos de la verificación
            - user_id (str): usuario a verificar
            - security_level (str): nivel de seguridad
            - required_sequence (list[str], opcional): secuencia de gestos
            - ip_address (str): IP del cliente
            - session_token (str, opcional): token del Plugin
            - callback_url (str, opcional): URL de retorno

    Returns:
        AuthenticationStartResponse:
            - session_id (str): identificador de la sesión
            - mode (str): "verification"
            - user_id (str): usuario verificado
            - security_level (str): nivel de seguridad aplicado
            - message (str): estado del inicio
    """
    try:
...
async def start_identification(request: IdentificationStartRequest):
    """
    Inicia una sesión de identificación biométrica 1:N.

    Args:
        request (IdentificationStartRequest): datos de la identificación
            - security_level (str): nivel de seguridad
            - ip_address (str): IP del cliente
            - session_token (str, opcional): token del Plugin
            - callback_url (str, opcional): URL de retorno
    ...
    Returns:
        AuthenticationStartResponse:
            ...
            - user_id (None): no aplica en identificación 1:N
            - security_level (str): nivel de seguridad aplicado
            - message (str): estado del inicio
    """
    try:
...
async def process_authentication_frame(session_id: str, request: ProcessFrameRequest):
    """
    Procesa un frame enviado desde el frontend para una sesión de autenticación.

    Args:
        session_id (str): identificador de la sesión
        request (ProcessFrameRequest): frame en base64

    Returns:
        dict:
            - status (str): estado del proceso
            - phase (str): fase actual
            - progress (float): avance de la autenticación
            - message (str): mensaje de estado
            - frames_processed (int): frames procesados
            - frame_processed (bool): indica si el frame fue procesado
            - frame (str|None): frame con overlay en base64
            - current_gesture (str|None): gesto detectado
            - gesture_confidence (float|None): confianza del gesto
            - required_sequence (list[str]|None): secuencia esperada (1:1)
            - captured_sequence (list[str]|None): secuencia capturada
            - sequence_complete (bool|None): secuencia completa (1:N)
            - gestures_needed (int|None): gestos requeridos (1:N)
            - is_locked (bool): indicador de bloqueo
            - lockout_info (dict|None): información de bloqueo
    """
    try:
        ...
        #  Decodificar frame del frontend
        try:
...
async def cancel_authentication(session_id: str):
    """
    Cancela una sesión de autenticación activa.
    ...
    Returns:
        dict:
            - cancelled (bool): indica si la sesión fue cancelada
            - session_id (str): id de la sesión cancelada
            - message (str): confirmación de la cancelación
    """
    try:
        auth_system = get_real_authentication_system()
...
async def get_available_users():
    """
    Obtiene la lista de usuarios disponibles para autenticación.

    Returns:
        dict:
            - users (list): usuarios disponibles
...
async def get_user_by_email(email: str):
    """
    Busca un usuario disponible para autenticación a partir de su email.
    ...
    Returns:
        dict:
            - success (bool): indica si el usuario fue encontrado
            - user (dict): información del usuario encontrado
    """
    try:
        auth_system = get_real_authentication_system()
...
async def initialize_authentication_system():
    """
    Inicializa el sistema de autenticación si aún no está activo.

    Returns:
        dict:
            - initialized (bool): indica si el sistema quedó inicializado
            - message (str): estado del proceso de inicialización
    """
    try:
        auth_system = get_real_authentication_system()
